...
| "Continue with Google" shows "Google Sign-In credentials not found …" | The **Web** client ID isn't reaching the running build. Three common causes: (1) the installed APK was built *before* the ID was added — make a **new** build; (2) you used the **Android** client ID — only the **Web application** client ID goes in `EXPO_PUBLIC_CLERK_GOOGLE_WEB_CLIENT_ID`; (3) the value never made it into `expo.extra`. `app.config.js` now injects it into `extra` from the env at build time (Clerk's `useSignInWithGoogle` reads `Constants.expoConfig.extra` first, then `process.env`), so just set the env var in `.env`/`eas.json` and rebuild. |
